...
int count;
FILE *fp;
void myvideo_back(MEDIA_BUFFER mb)
{
    count++;
    printf("%d已编码\n",count);
    if(count <300)
...
int main()
{
    ...
    aenc_attr.stAencG711A.u32SampleRate =16000;
    ...
    aenc_chn.s32DevId = 0;
    aenc_chn.s32ChnId = 0;
    RK_MPI_SYS_RegisterOutCb(&aenc_chn, myvideo_back);
...
int main()
{
    ...
    adec_attr.stAdecG711A.u32Channels =2;
    adec_attr.stAdecG711A.u32SampleRate =16000;
    ...
    des_chn.s32ChnId = 0;
    RK_MPI_SYS_Bind(&src_chn,&des_chn);
    MB_AUDIO_INFO_S stSampleInfo = {2, 16000,1024,RK_SAMPLE_FMT_S16};
    while(1)
    {
        mb = RK_MPI_MB_CreateAudioBufferExt(&stSampleInfo, RK_FALSE, 0);
        byte_count =fread(RK_MPI_MB_GetPtr(mb),1,RK_MPI_MB_GetSize(mb),fp);
        if(byte_count <= 0)
        {
            fclose(fp);
            break;
        }
        RK_MPI_MB_SetSize(mb, byte_count);
